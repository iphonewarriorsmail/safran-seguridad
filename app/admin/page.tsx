      ...
      <div className="bg-surface border border-border rounded-2xl overflow-hidden">
        ...
        {error ? (
          <div className="p-6 text-rose-400">
            Error al cargar las consultas. Verifica que la tabla 'leads' exista y tenga la columna 'leida' (boolean).
          </div>
        ) : (
          <div className="divide-y divide-border">
            {leads && leads.length > 0 ? (
              leads.map((lead) => (
                <div key={lead.id} className={`p-6 flex flex-col md:flex-row gap-4 transition-colors ${lead.leida ? 'opacity-70 bg-surface/50' : 'bg-surface-hover/30'}`}>
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-bold text-lg">{lead.nombre}</h3>
                      <span className="text-xs bg-border px-2 py-1 rounded-full">{lead.servicio}</span>
                      <span className="text-xs text-muted">{new Date(lead.created_at).toLocaleDateString()}</span>
                    </div>
                    <p className="text-muted text-sm mb-3">{lead.mensaje}</p>
                  ...
                  <div className="flex items-center justify-end md:w-32">
                    <form action={markLeadAsRead.bind(null, lead.id, lead.leida)}>
                      <button 
                        type="submit" 
                        title={lead.leida ? 'Marcar como no leída' : 'Marcar como leída'}
                        className={`p-3 rounded-xl transition-all ${lead.leida ? 'text-muted hover:bg-surface-hover hover:text-foreground' : 'bg-accent/10 text-accent hover:bg-accent/20 hover:text-accent'}`}
                      >
                        {lead.leida ? <MailOpen className="w-6 h-6" /> : <Mail className="w-6 h-6" />}
                      </button>
                    </form>
                  </div>
                </div>
